...
- Plugins: jekyll-feed (0.17.0), jekyll-polyglot (multi-language), jekyll-seo-tag (2.8.0), jekyll-sitemap (1.4.0)
...
- `main.css`: Master stylesheet that imports all modular files and contains the live design tokens
...
- jekyll-sitemap (1.4.0): Generates sitemap.xml
...
- sitemap.xml is excluded from localization (single sitemap at root)
...
See `docs/VOICE_AND_STYLE.md` for the full voice and style guide. Key principles:
...
- Don't claim expertise the reader has no reason to believe yet. Start with the honest label (digital marketer) and let depth emerge through the work.
- Dry humor through stating the obvious. Don't try to be funny.
- No em dashes or en dashes. No exclamation marks. No "journey," "insights," or "passionate about."
- No implicit promises or commitments about what the site will contain.
- Sentence case for all headings, buttons, and labels. "Latest projects" not "Latest Projects." Single-word labels (Home, Blog, About) are fine capitalized.
- Finnish content should follow the same voice principles. Keep it natural, not stiff or overly formal.
...
- head.html must not contain manual OG/Twitter/canonical meta tags; jekyll-seo-tag handles all of these via `{% seo %}`. Adding manual tags creates duplicates.
...
- `{{ I18n_Headers }}` must use double braces and capital H; `{% I18n_headers %}` causes "Unknown tag" build errors
